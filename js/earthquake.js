 ...
 * @Last modified time: 2018-05-07T11:34:28-04:00
 */
...
function createScatter(data, currentkey) {
  ...
  y2.domain([0, d3.max(expenseMetrics, function(d) {
  ...
  dots.selectAll("dot")
    ...
    .attr("class", "bar")
    .attr("x", function(d) {
      return x2(parseDate1(d.key));
    })
    .attr("y", function(d) {
      return y2(d.value.count);
    })
    .attr("width", 15)
    .attr("height", function(d) {
      return height2 - y2(d.value.count);
    })
    .style("fill", "#e6550d")
  // draw the scatter
  //dots.selectAll("dot")
